...
@section('title', 'Tambah Komponen Baru')
...
<div class="mb-4">
    ...
    <h2 class="display-font mt-1 fw-extrabold" style="text-transform: uppercase;">Tambah Komponen Baru</h2>
</div>
                ...
                <form action="{{ route('admin.components.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="subcategory" id="subcategory" class="form-select form-control-custom @error('subcategory') is-invalid @enderror" required>
                                ...
                                <option value="Groupsets" {{ old('subcategory') == 'Groupsets' ? 'selected' : '' }}>Groupsets</option>
                                <option value="Wheelsets" {{ old('subcategory') == 'Wheelsets' ? 'selected' : '' }}>Wheelsets</option>
                                <option value="Cockpit Parts" {{ old('subcategory') == 'Cockpit Parts' ? 'selected' : '' }}>Cockpit Parts</option>
                                <option value="Saddles" {{ old('subcategory') == 'Saddles' ? 'selected' : '' }}>Saddles</option>
                                <option value="Other Components" {{ old('subcategory') == 'Other Components' ? 'selected' : '' }}>Other Components</option>
                            </select>
                            @error('subcategory')
                                <div class="invalid-feedback fw-semibold">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="specs[Brand]" id="specs_brand" class="form-control form-control-custom" placeholder="Contoh: Shimano / SRAM / DT Swiss" value="{{ old('specs.Brand') }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="specs_weight" class="form-label form-label-custom">Berat Komponen</label>
                            <input type="text" name="specs[Weight]" id="specs_weight" class="form-control form-control-custom" placeholder="Contoh: 2,438 grams" value="{{ old('specs.Weight') }}">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="specs[Material]" id="specs_material" class="form-control form-control-custom" placeholder="Contoh: Carbon Fiber / Dura-Ace R9200" value="{{ old('specs.Material') }}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="specs_other" class="form-label form-label-custom">Drivetrain Speed / Detail Hub (Opsional)</label>
                            <input type="text" name="specs[Drivetrain Speed]" id="specs_other" class="form-control form-control-custom" placeholder="Contoh: 2x12-Speed Electronic" value="{{ old('specs.Drivetrain_Speed') }}">
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 justify-content-end mt-4 border-top pt-3">
                        ...
                        <button type="submit" class="btn btn-turquoise py-2 px-4"><i class="bi bi-save me-2"></i> Simpan Komponen</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="col-lg-5">
        <div class="card card-custom bg-light border-0">
            ...
            <div class="card-body text-muted" style="font-size: 0.85rem; line-height: 1.6;">
                <p>Silakan ikuti instruksi berikut untuk memastikan kualitas data produk komponen sepeda yang dijual terpisah:</p>
                <ul class="ps-3 mb-4">
                    <li class="mb-2"><strong>Otomatisasi Slug</strong>: URL slug (handle) untuk navigasi detail produk akan otomatis dibuat berdasarkan Nama Komponen (e.g. "Shimano Dura-Ace Groupset" menjadi "shimano-dura-ace-groupset").</li>
                    <li class="mb-2"><strong>Kategori Komponen</strong>: Semua produk suku cadang yang diinputkan akan langsung dipetakan ke kategori <code>Components</code> di database.</li>
                    <li class="mb-2"><strong>Dukungan Gambar</strong>: Sistem akan mendeteksi sub-kategori pilihan Anda dan mengarahkan path gambar ke asset lokal yang sesuai:
                        <ul class="ps-3 mt-1">
                            <li>Wheelsets ➔ <code>/images/category_gravel.png</code></li>
                            <li>Kategori Lainnya ➔ <code>/images/hero_road_bike.png</code></li>
                        </ul>
                    </li>
                </ul>
                <div class="p-3 bg-white border border-info border-start border-4">
                    <strong class="text-dark d-block mb-1"><i class="bi bi-cart3 text-info me-1"></i> Penjualan Eceran:</strong>
                    Berbeda dari sepeda (bikes) yang dijual full set, komponen/parts ini ditujukan bagi pelanggan yang ingin membangun rakitan custom atau mengganti komponen sepeda mereka secara terpisah.
                </div>
            </div>
        </div>
    </div>
